...
/**
 * Renders the Television AI identity, and nothing at all when there is no profile.
 *
 * A per-category component over a shared `<AiIdentitySection category>`: the presentation lives in
 * `AiIdentityCard`, and this file exists so a genuinely television-specific addition — season-count framing is a
 ...
 */
export default function AiTvIdentitySection({ enabled = false }: { enabled?: boolean }) {
  const profile = useAiTasteProfile('tv', { enabled });

  if (!profile) {
    return null;
  }

  return <AiIdentityCard eyebrow="Television Identity" profile={profile} />;
}
