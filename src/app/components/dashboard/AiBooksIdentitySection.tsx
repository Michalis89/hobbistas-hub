...
/**
 * Renders the Reading AI identity, and nothing at all when there is no profile.
 *
 * A per-category component over a shared `<AiIdentitySection category>`: the presentation lives in
 * `AiIdentityCard`, and this file exists so a genuinely reading-specific addition — page-count or author framing is a
 ...
 */
export default function AiBooksIdentitySection({ enabled = false }: { enabled?: boolean }) {
  const profile = useAiTasteProfile('books', { enabled });

  if (!profile) {
    return null;
  }

  return <AiIdentityCard eyebrow="Reading Identity" profile={profile} />;
}
